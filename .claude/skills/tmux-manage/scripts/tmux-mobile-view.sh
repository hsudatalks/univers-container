...
auto_start_dependencies() {
    ...
    # 检查 univers-agents
    if ! command tmux has-session -t "univers-agents" 2>/dev/null; then
        log_info "启动 univers-agents..."
        local agents_script="$CORE_DIR/ops/tmux-agents.sh"
...
start_session() {
    ...
    # 连接到各服务 (pane 1=top, 2=middle, 3=bottom - 因为 pane-base-index=1)
    ...
    # 连接到各 QA 服务 (pane 1=top, 2=middle, 3=bottom - 因为 pane-base-index=1)
