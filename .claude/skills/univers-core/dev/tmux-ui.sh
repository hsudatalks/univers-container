...
# 配置
SESSION_NAME="univers-ui"
WINDOW_NAME="storybook"
# 解析符号链接获取真实脚本路径
...
start_service() {
    ...
    case "$mode" in
        prod)
            start_command="pnpm ui:storybook:prod"
            log_info "启动模式: 生产环境配置"
            ;;
        safe)
            start_command="pnpm ui:dev:safe"
            log_info "启动模式: 安全模式"
            ;;
        *)
            start_command="pnpm ui:dev"
            log_info "启动模式: 开发环境 (Storybook + HMR)"
            ;;
    esac
    ...
    # 加载会话专属状态栏配置
    local statusbar_config="$STATUSBAR_DIR/ui-statusbar.conf"
    if [ -f "$statusbar_config" ]; then
        ...
        log_info "已加载状态栏配置: ui-statusbar.conf"
    else
...
show_status() {
    ...
    echo "═══════════════════════════════════════════════════════════"
    echo "  Univers UI Status (Storybook)"
    echo "═══════════════════════════════════════════════════════════"
    echo ""
    ...
    if session_exists; then
        ...
        if is_service_running; then
            log_success "UI服务: 运行中"

            # 尝试检查端口
            if command -v netstat &> /dev/null; then
                echo ""
                echo "监听端口:"
                netstat -tuln | grep -E ":6006" || echo "  未检测到Storybook端口 (6006)"
            fi
        else
            log_warning "UI服务: 未运行"
        fi
...
show_help() {
    cat << EOF
Univers UI Tmux Manager (Storybook)

用法:
  $0 <command> [options]

命令:
  start [mode]    启动UI服务 (默认dev模式)
                  模式: dev, prod, safe
  stop            停止UI服务
  restart [mode]  重启UI服务
  attach          连接到会话
...
提示:
  - UI服务在tmux后台运行，关闭终端也不会停止
  - 使用 'attach' 命令查看实时输出
  - 使用 'logs' 命令查看历史日志
  - Storybook通常在 http://localhost:6006 访问
