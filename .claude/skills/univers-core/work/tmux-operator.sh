#!/usr/bin/env bash
#
# Tmux Operator Session
# AI 运维终端 - 用于 hvac-operation 项目
#
...
do_start() {
    ...
    log_service "创建 Operator 运维终端会话: $SESSION_NAME"
    ...
    if [ -f "$STATUSBAR_CONFIG" ]; then
        ...
        log_service "已加载状态栏配置"
    fi

    # 发送欢迎消息
    send_welcome "$SESSION_NAME:$WINDOW_NAME" \
        "Operator Terminal - hvac-operation" \
        "Project: $PROJECT_ROOT"

    log_success "Operator 运维终端已启动"
    echo ""
    echo "使用以下命令:"
    ...
    echo "  $0 status   - 查看状态"
    echo ""
    log_info "这是一个持久化的运维终端，关闭终端窗口也不会消失"
}
...
do_attach() {
    ...
    log_info "连接到 Operator 会话... (按 Ctrl+B 然后 D 退出)"
